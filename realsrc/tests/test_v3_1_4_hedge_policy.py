...
def _setup():
    ...
    ST.HEDGE_SOFT_INITIAL_RATIO = 0.50
...
def _hedge(current=0.0, target=0.02, side="buy", venue="BINANCE", data_gap=None):
...
def _plan(current=0.0, target=0.02, risk=None, snap=None):
    return ST._hedge_policy_plan(
        snap or _snap(), _hedge(current=current, target=target), risk or _risk(), NOW)


def test_soft_first_trigger_targets_half_full_target():
    _setup()
    h = _plan(current=0.0, target=0.02)
    assert h["policy_detail"]["trigger_state"] == "SOFT"
    assert h["policy_detail"]["eff_target_qty"] == 0.01
...
def test_late_fill_current_absorbs_delta_and_prevents_overhedge():
    ...
    assert h["policy_detail"]["eff_target_qty"] == 0.01
    assert h["action"]["action"] == "HEDGE_HOLD"
    assert h["policy_detail"]["reason"] == "LOT_DEADBAND"
