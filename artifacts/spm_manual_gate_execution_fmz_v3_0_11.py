...
# Deribit S:PM 垂直信用价差卖方执行链 v3.0.11-manual-gate（FMZ 单文件；单一 run_cycle 主链 + 交互控制台 + 对冲生命周期）
...
STRATEGY_VERSION = "3.0.11-manual-gate"
...
def _post_maker_once(side, instrument, amount, price, label, meta=None):
...
def exec_entry_campaign_step(prot_inst, short_inst, amount, credit_floor, max_tick_steps,
    ...
    if short_cap > 1e-12:                                       # 短腿数量 ≤ 已成交保护腿量
        short_fill = _post_maker_once("sell", short_inst, short_cap, short_price,
