...
# Deribit S:PM 垂直信用价差卖方执行链 v3.0.1-manual-gate（FMZ 单文件；单一 run_cycle 主链 + 交互控制台 + 对冲生命周期）
...
v3.0.1 收束配置入口：执行层不再要求填写审计卡资料；信号层人工确认后，
只需打开 MANUAL_PLANNING_ALLOWED。出方案不交易由交易门关闭实现，不再区分 PLAN/ORDER。
"""

# ===== 当前版本号（显示于启动日志/面板/合成文件头）=====
# 3.0.1-manual-gate：独立人工审计门执行层，收束配置入口，完整主链空跑/执行。
