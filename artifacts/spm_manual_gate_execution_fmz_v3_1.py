...
# Deribit S:PM 垂直信用价差卖方执行链 v3.1-manual-gate（FMZ 单文件；单一 run_cycle 主链 + 交互控制台 + 对冲生命周期）
...
STRATEGY_VERSION = "3.1-manual-gate"
...
def _position_manage_overview_table(ctx):
    ...
    return {"type": "table", "title": "持仓总览", "cols": ["项目", "值", "备注"], "rows": [
        ...
        ["盈亏平衡价", _num(pd.get("breakeven"), small=2, big=2), "旧持仓缺失则为数据缺口"],
...
def _take_profit_budget_table(ctx):
    ...
    if not tp.get("quote_ok", True) and tp.get("quote_gap"):
        status += "｜" + str(tp.get("quote_gap"))
    risk_auth = "已授权" if risk.get("authorized") else "未授权"
    risk_within = "可越价执行" if risk.get("within") else "不可越价/预算不足"
    return {"type": "table", "title": "止盈/退出预算", "cols": ["项目", "值", "备注"], "rows": [
        ...
        ["风险退出授权", "%s ｜ 码 %s ｜ %s" % (risk_auth, risk.get("auth_code") or "—", risk.get("budget_source") or "—"), risk_within],
...
def _risk_hedge_table(ctx):
    ...
    price_line = (_num(h.get("hedge_price_line"), small=2, big=2)
                  if h.get("hedge_price_line") is not None else "概率触发，无固定价线")
    reduce_only = "reduce_only=%s" % ("是" if h.get("reduce_only") else "否")
...
def disp_pipeline_table(ctx):
    ...
    rows = [
        ...
        ["退出模块", "%s ｜ %s ｜ 风险码 %s" % (
            ...
            g("risk_exit_auth_code") or (g("risk_exit_detail") or {}).get("auth_code") or "—")],
    ]
...
def _build_position_detail(snap, quote_fn, now_ms, exit_state=None, in_flight=None):
    if not snap:
        return None
...
def _build_risk_exit_detail(auth, risk_code, risk_exit, exit_budget, exit_cap, exit_within):
    policy = (auth or {}).get("policy_code")
    max_spend = (auth or {}).get("max_exit_spend")
    risk_authorized = bool(policy == POLICY_RISK_EXIT and (auth or {}).get("authorization_state") == "AUTHORIZED")
    return {
        "auth_code": risk_code,
        "authorized": risk_authorized,
        "policy_code": policy,
        "max_exit_spend": max_spend,
        "budget_source": "RISK_EXIT_MAX_SPEND" if isinstance(max_spend, (int, float)) and max_spend > 0 else "未授权/无独立预算",
        "remaining_budget": exit_budget,
        "price_cap": exit_cap,
        "within": exit_within,
...
def manage_cycle(now_ms):
    ...
    **退出活动期禁新增对冲敞口**（只许 reduce/unwind）。退出/对冲/回收真实下单均受各自门控，默认空跑。"""
    ...
    position_detail = _build_position_detail(snap, quote_fn, now_ms, long_state or exit_state, in_flight)
    hedge_detail = _build_hedge_detail(hedge, risk)
    risk_exit_detail = _build_risk_exit_detail(
        auth, risk_code, risk_exit, exit_budget, exit_cap, exit_within)
