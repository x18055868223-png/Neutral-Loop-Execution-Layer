...
# Deribit S:PM 垂直信用价差卖方执行链 v3.0.2-manual-gate（FMZ 单文件；单一 run_cycle 主链 + 交互控制台 + 对冲生命周期）
...
v3.0.2 增加链路心跳与候选枚举诊断：人工放行后可在日志/面板看到卡在哪个过滤门。
执行层仍只需打开 MANUAL_PLANNING_ALLOWED；出方案不交易由交易门关闭实现。
"""

# ===== 当前版本号（显示于启动日志/面板/合成文件头）=====
# 3.0.2-manual-gate：独立人工审计门执行层，收束配置入口，完整主链空跑/执行。
...
def disp_status_panel(ctx, note=""):
    ...
    if ctx.get("menu"):                           # 策略选择明细（已并入到期/盈亏平衡价，无独立概要表）
        tables.append(disp_menu_table(ctx["menu"], None, ctx.get("spot")))
