...
# Deribit S:PM 垂直信用价差卖方执行链 v3.0.7-manual-gate（FMZ 单文件；单一 run_cycle 主链 + 交互控制台 + 对冲生命周期）
...
v3.0.7 使用 GEX Monitor BTC Metric API 的 iv/rv ratio rank 作为轻量 VRP_CONTEXT，
并在方案锁定后把预览/合理性检查聚焦到锁定方案。
执行层仍只需打开 MANUAL_PLANNING_ALLOWED；出方案不交易由交易门关闭实现。
"""

# ===== 当前版本号（显示于启动日志/面板/合成文件头）=====
# 3.0.7-manual-gate：PLAN_LOCKED 展示锁定方案，不再沿用固定库首项预览。
STRATEGY_VERSION = "3.0.10-manual-gate"
...
MANUAL_PLANNING_ALLOWED = False     # 信号层人工确认后改 True；False=等待，不生成可执行确认码
DIRECTION_BIAS      = "SHORT_CALL"  # SHORT_CALL=偏空卖 Call；SHORT_PUT=偏多卖 Put
...
CANDIDATE_MENU_SIZE = 10                # 候选/确认码最多展示条数
MENU_SIZE = CANDIDATE_MENU_SIZE         # 内部兼容别名；不要作为日常入口修改
STATUS_MENU_SIZE = 50                   # 状态栏最多展示候选方案条数；足够大以完整显示本轮备选库
PLAN_LIBRARY_FREEZE = True             # True=首次扫描后固定备选方案库，避免实时重排/抖动
PLAN_LIBRARY_REFRESH_COMMAND_ONLY = True  # 固定库需要改参数/重启/恢复命令后才重建；不按行情自动刷新

# ===== 轻量 VRP_CONTEXT 数据源：只校验有效性，不按 rank 高低过滤候选 =====
GEX_METRIC_API_BASE = "http://13.231.16.198:8000"
GEX_METRIC_API_BEARER_TOKEN = "7WkM4LBAha7di0KMCtgty3NwdQcNXI5-j3o8MymkGiE"
...
ALLOW_HEDGE_TRADING   = False     # Binance BTCUSDC / Deribit BTC-PERPETUAL 对冲开 / 加 / 减仓
...
PLAN_REFRESH_SECONDS = 45         # 主链重算候选库的最小间隔(秒)：节流 API + 防刷屏
TRACE_LOG_SECONDS = 15            # 链路心跳日志间隔；0=关闭周期心跳
TRACE_PLAN_STAGES = True          # 候选构建阶段日志：拉链、筛到期、筛短腿、S:PM、VRP
TRACE_PLAN_DETAIL_ROWS = 10         # 每次生成候选后写入日志的候选明细条数
...
# ===== 对冲（§10；默认 Binance BTCUSDC，目标随剩余卖方敞口；压尾部非全 delta-neutral）=====
...
# ----- 对冲场所（默认 Binance；Deribit BTC-PERPETUAL 仅作兼容）-----
HEDGE_VENUE = "BINANCE"                # "BINANCE" | "DERIBIT"
HEDGE_BINANCE_INSTRUMENT = "BTCUSDC"   # 币安 USDC 本位永续（线性）
...
def validate_config():
    """启动期配置自检，返回错误列表（空=通过）。"""
    errs = []
    _profile = normalize_run_profile()
    if _profile not in ("TEST", "LIVE"):
    ...
    if CANDIDATE_MENU_SIZE < 1:
        errs.append("CANDIDATE_MENU_SIZE 必须 >= 1")
    ...
    if TRACE_LOG_SECONDS < 0:
        errs.append("TRACE_LOG_SECONDS 必须 >= 0")
    if TRACE_PLAN_DETAIL_ROWS < 1:
        errs.append("TRACE_PLAN_DETAIL_ROWS 必须 >= 1")
    if not isinstance(TRACE_PLAN_STAGES, bool):
        errs.append("TRACE_PLAN_STAGES 必须为布尔值")
    ...
    for _m in live_checklist_missing():
        errs.append("RUN_PROFILE=LIVE requires " + _m)
    _gates = effective_trading_gates()
    _live_gates = _gates["allow_entry"] or _gates["allow_exit"] or _gates["allow_hedge"]
    if _live_gates and not DRY_RUN_PASSED:
        errs.append("DRY_RUN_PASSED=False; live trading gates must stay disabled")
    if _gates["allow_entry"]:
        if not _gates["allow_exit"]:
...
def build_manual_context(now_ms, planning_allowed, direction_bias, dte_hours, delta_range,
    ...
    sig = manual_config_signature(
        ...
        amount, audit_card_id, audit_note, ttl_min, risk_policy, market_context,
        vrp_context_status)
    ...
    return {
        ...
        "risk_policy": risk_policy or {},
        "market_context": market_context or {},
        "vrp_context_status": vrp_context_status or "VRP_CONTEXT_MISSING",
    }
...
def build_gex_vrp_context(payload, direction_bias):
    if isinstance(payload, str):
        try:
            payload = json.loads(payload)
        except Exception:
            return {"valid": False, "status": "VRP_CONTEXT_INVALID_JSON", "market_context": None}
    if not isinstance(payload, dict):
        return {"valid": False, "status": "VRP_CONTEXT_INVALID_DATA", "market_context": None}
    if payload.get("availability") != "ready":
        return {"valid": False, "status": "VRP_CONTEXT_NOT_READY", "market_context": None}
    ...
    ivrv = _float_or_none(metric.get("value"))
    rank_pct = _float_or_none(metric.get("rank_pct"))
    percentile = _float_or_none(metric.get("percentile"))
    sample_count = _float_or_none(metric.get("sample_count"))
    if ivrv is None or rank_pct is None:
        return {"valid": False, "status": "VRP_CONTEXT_MISSING_FIELDS:volatility.iv_rv_ratio_rank",
                "market_context": None}
    ...
    mc = {
        ...
        "iv_rv_percentile": percentile,
        "sample_count": sample_count,
        "quality": metric.get("quality"),
        "window": (payload.get("rank") or {}).get("window") or {},
    }
    return {"valid": True, "status": "VRP_CONTEXT_VALID", "market_context": mc}


def fetch_gex_vrp_context(direction_bias):
    url = GEX_METRIC_API_BASE.rstrip("/") + "/v1/info"
    headers = {"Authorization": "Bearer " + GEX_METRIC_API_BEARER_TOKEN}
    try:
        try:
            from urllib.request import Request, urlopen
        except ImportError:  # pragma: no cover - FMZ old Python fallback
        ...
        return {"valid": False, "status": "VRP_CONTEXT_API_ERROR:%s" % exc, "market_context": None}
    return build_gex_vrp_context(raw, direction_bias)
...
def _record_last_command(raw, command, status, outcome, now_ts):
    rec = {"raw": raw, "type": (command or {}).get("type"),
           "name": (command or {}).get("name"), "arg": (command or {}).get("arg"),
           "status": status, "outcome": outcome, "ts": now_ts}
    _G(_LAST_COMMAND_KEY, rec)
    Log("[cmd]", "raw=%s" % raw, "type=%s" % rec["type"],
        "arg=%s" % (rec["arg"] or "—"), "status=%s" % status,
        "outcome=%s" % outcome)
    return rec
...
REASON_CN = {
    ...
    "PLAN_MENU_READY": "候选已生成；交易门关闭时只展示，不真实下单",
...
def disp_manual_hint(manual_gate_state):
    ...
    if manual_gate_state == "PLANNING_ALLOWED":
        return "信号层已人工确认；按方向、数量和次级默认参数生成候选"
    if manual_gate_state == "WAIT_MANUAL_AUDIT_GATE":
        return "信号层人工确认后，将 MANUAL_PLANNING_ALLOWED 改为 True 即可生成候选"
    if manual_gate_state == "MANUAL_CONTEXT_INVALID":
        return "方向、数量或次级默认参数无效/过期，需修正后重建候选"
    return "—"
...
def _overview_table(ctx):
    ...
    return {
        ...
        "rows": [
            ...
            ["候选/确认码", "展示%s / 可锁定%s / VRP阻断%s / %s" % (
                g("display_candidates_count") or 0, g("lockable_candidates_count") or 0,
                g("plan_vrp_blocked") or 0,
                "固定库" if g("plan_library_frozen") else (g("menu_source") or "实时"))],
            ["VRP_CONTEXT", "%s ｜ iv/rv=%s rank=%s%%" % (
                g("vrp_context_status") or "—",
                _num((g("vrp_context") or {}).get("iv_rv_ratio"), small=2, big=2),
                _num((g("vrp_context") or {}).get("iv_rv_rank_pct"), small=1, big=1))],
            ["计划链路原因", g("not_lockable_reason") or g("plan_build_reason") or "—"],
...
def disp_diag_line(diag):
    ...
    if not diag:
        return "—"
    return ("扫描%s → delta外%s/无报价%s/薄%s/宽%s/无保护%s/执行不可行%s → 候选%s → 进库%s → 合格%s" % (
        diag.get("短腿扫描", 0), diag.get("delta区间外", 0), diag.get("无报价/无买盘", 0),
        diag.get("权利金过薄", 0), diag.get("价差过宽", 0), diag.get("无合格保护腿(腿宽内)", 0),
        diag.get("执行不可行", 0), diag.get("生成候选", 0), diag.get("进入菜单", 0), diag.get("合格", 0)))
...
def disp_menu_table(menu, selected_no, spot):
    ...
    for p in menu:
        ...
        rows.append([
            "%s%s" % (star, g("id")), code, tags, g("mode_cn") or "—", qihao, dte,
    ...
    return {
        ...
        "cols": ["编号", "确认码/锁定状态", "推荐", "模式", "期号(短/保护)", "到期", "短行权(Δ)", "保护行权",
...
_PHASE_CN = {
    ...
    "PLAN_MENU_READY": "候选生成/过滤", "RECOMMEND_READY": "方案库就绪·待硬授权", "HARD_APPROVAL_WAIT": "待计划硬授权",
...
_HINTS = {
    "WAIT_MANUAL_AUDIT_GATE": "等待信号层人工确认；确认后只需开启 MANUAL_PLANNING_ALLOWED，持仓管理继续",
    "MANUAL_GATE": "人工审计门已放行：进场依据 DIRECTION_BIAS、ORDER_AMOUNT 与次级默认参数",
    "PLAN_MENU_READY": "正在生成/过滤候选；交易门关闭时只展示，不真实下单",
...
def disp_pipeline_table(ctx):
    """完整主链状态总览：计划、确认码、预提交、执行、记账、对冲、退出模块一屏可见。"""
    g = ctx.get
    ...
    arb = g("action_arb") or {}
    rows = [
        ["计划轮", "reason=%s ｜ diag=%s" % (g("plan_build_reason") or "—", disp_diag_line(g("enum_diag")) if g("enum_diag") else "—")],
        ...
        ["记账/恢复", "state=%s ｜ recovery=%s ｜ reconciled=%s" % (
            disp_state_cn(g("state")), g("recovery_state") or "OK", g("reconciled") if g("reconciled") is not None else "—")],
        ["对冲模块", g("hedge_state") or ("risk=%s" % (g("risk_state") or "—"))],
        ["退出模块", "%s ｜ TP=%s ｜ risk_code=%s" % (
            g("exit_campaign_state") or "—", g("take_profit_ratio") or "—", g("risk_exit_auth_code") or "—")],
    ]
...
def disp_console_table(ctx):
    ...
    if g("plan_build_reason") or g("not_lockable_reason") or g("display_candidates_count") is not None:
        rows.append(["候选链路", "展示%s / 可锁定%s / VRP阻断%s ｜ %s" % (
            g("display_candidates_count") or 0, g("lockable_candidates_count") or 0,
            g("plan_vrp_blocked") or 0, g("not_lockable_reason") or g("plan_build_reason") or "—")])
    if g("enum_diag"):
        rows.append(["枚举漏斗", disp_diag_line(g("enum_diag"))])
    if g("last_command_raw"):
        rows.append(["最近交互", "%s ｜ %s/%s ｜ %s" % (
            g("last_command_raw"), g("last_command") or "—",
            g("last_command_status") or "—", g("last_command_outcome") or "—")])
...
def disp_status_panel(ctx, note=""):
    ...
    if ctx.get("menu"):                           # 候选方案即使不可锁定也展示，便于定位 VRP/预算/确认码链路
        tables.append(disp_menu_table((ctx["menu"] or [])[:max(1, STATUS_MENU_SIZE)], None, ctx.get("spot")))
...
def disp_log_summary(ctx, note=""):
    ...
    diag = disp_diag_line(g("enum_diag")) if g("enum_diag") else "—"
    return ("%s ｜ phase=%s ｜ 候选%s/可锁%s/VRP阻%s ｜ 原因=%s ｜ 漏斗=%s ｜ 短 %s@%s ｜ 保 %s@%s ｜ 释放 %s ｜ %s" % (
        note or "进场", g("console_phase") or "—", g("display_candidates_count") or 0,
        g("lockable_candidates_count") or 0, g("plan_vrp_blocked") or 0,
        g("not_lockable_reason") or g("plan_build_reason") or "—", diag,
        g("short_instrument") or "—", _num(g("short_mark")),
...
# ===================== module: hedge =====================
# -*- coding: utf-8 -*-
"""BTC-PERPETUAL / BTCUSDC 对冲生命周期（hedge_*）。纯函数，便于单测。
...
def _build_menu(now_ms, spot, manual_context=None, _external_unused=None):
    ...
    diag = {"短腿扫描": 0, "delta区间外": 0, "无报价/无买盘": 0, "权利金过薄": 0,
            ...
            "生成候选": 0, "进入菜单": 0, "合格": 0}
    _trace_plan("build_start", spot=spot, direction=manual_context.get("direction_bias") or DIRECTION_BIAS,
                dte="%s-%s" % (dte_min, dte_max), delta="%s-%s" % (dmin, dmax),
                width="%s-%s" % width_range, amount=amount)
    instruments = dbt_get_instruments(SETTLEMENT_CURRENCY, "option")
    _trace_plan("option_chain", instruments=len(instruments or []))
    if not instruments:
        _trace_plan("stop", reason="NO_INSTRUMENTS", diag=diag)
        return [], False, None, "NO_INSTRUMENTS", diag
    short_exps = legsel_expiries_in_band(instruments, dte_min, dte_max,
                                         now_ms, want_call)
    _trace_plan("expiry_filter", expiries=len(short_exps or {}), want_call=want_call)
    if not short_exps:
        _trace_plan("stop", reason="NO_SHORT_EXPIRY_IN_BAND", diag=diag)
    ...
    _trace_plan("account_spm", pm_ok=pm_ok, model=model)
    pref = (dmin + dmax) / 2.0
    ...
    if not prelim:
        reason = ("NO_EXECUTION_FEASIBLE" if diag["执行不可行"] else "NO_CANDIDATE")
        _trace_plan("candidate_filter_stop", reason=reason, diag=diag)
        return [], pm_ok, model, reason, diag
    _trace_plan("candidate_filter_pass", prelim=len(prelim), diag=diag)
    prelim.sort(key=lambda c: plan_prelim_score(c, PLAN_WEIGHTS), reverse=True)
    topk = prelim[:max(MENU_SIZE * 2, MENU_SIZE)]
    _trace_plan("spm_topk_start", topk=len(topk), menu_size=MENU_SIZE)
    ...
    for c in topk:                                    # 仅对 top-K 跑 S:PM（控制 API 调用）
        ...
        final.append(plan)
    if not final:
        _trace_plan("spm_topk_stop", reason="NO_EXECUTION_FEASIBLE", diag=diag)
    ...
    _trace_plan("menu_ready", menu=len(menu), qualified=diag["合格"], diag=diag)
...
_TRACE_LAST_KEY = "spm_trace_last_ms_v1"
_PLAN_TRACE_KEY = "spm_plan_trace_v1"


def _diag_compact(diag):
    if not diag:
        return "—"
    keys = ("短腿扫描", "delta区间外", "无报价/无买盘", "权利金过薄", "价差过宽",
            "无合格保护腿(腿宽内)", "执行不可行", "生成候选", "进入菜单", "合格", "VRP阻断")
    return ";".join("%s=%s" % (k, diag.get(k, 0)) for k in keys if k in diag)


def _trace_plan(stage, **kwargs):
    if not TRACE_PLAN_STAGES:
        return
    parts = []
    ...
    Log("[trace][plan]", stage, " ".join(parts))


def _trace_plan_candidates(menu, spot=None, pending=None, not_lockable_reason=None):
    """候选方案日志明细：状态栏显示之外，Log 里也保留方案与确认码/不可锁定原因。"""
    if not TRACE_PLAN_STAGES:
        return
    rows = list(menu or [])
    pend = pending or []
    codes = dict((c.get("id"), c.get("confirm_code")) for c in pend)
    ...
    Log("[trace][plan_candidates]",
        "display=%s" % len(rows),
        "codes=%s" % (";".join("#%s:%s" % (k, v) for k, v in codes.items()) or "—"),
        "not_lockable=%s" % (not_lockable_reason or "—"))
    for p in rows[:max(1, TRACE_PLAN_DETAIL_ROWS)]:
        dist = "—"
        ss = p.get("short_strike")
        if isinstance(ss, (int, float)) and spot:
            dist = "%+.1f%%" % ((ss - spot) / spot * 100.0)
        Log("[trace][plan_candidate]",
            "#%s" % (p.get("id") or "—"),
            "expiry=%s" % (p.get("short_expiry_label") or "—"),
            "short=%s" % (p.get("short_instrument") or "—"),
            "K=%s" % n(p.get("short_strike"), 0),
            "delta=%s" % n(p.get("short_delta"), 3),
            "dte_h=%s" % n(p.get("short_dte_hours"), 2),
            "prot=%s" % (p.get("protection_instrument") or "—"),
            "protK=%s" % n(p.get("protection_strike"), 0),
            "width=%s" % n(p.get("width"), 0),
            "dist=%s" % dist,
            "credit=%s" % n(p.get("net_credit_effective"), 8),
            "rr=%s" % n(p.get("rr"), 3),
            "relief=%s" % n(p.get("margin_relief_ratio"), 3),
            "ef=%s" % (p.get("execution_feasibility_grade") or "—"),
            "confirm=%s" % (codes.get(p.get("id")) or "—"),
            "qualified=%s" % ("Y" if p.get("qualified") else "N:" + str(p.get("reject_reason") or "")))
...
def _maybe_trace_cycle(ctx, now_ms):
    if TRACE_LOG_SECONDS <= 0:
        return
    try:
        last = int(_G(_TRACE_LAST_KEY) or 0)
    except Exception:
        last = 0
    force = bool(ctx.get("plan_build_attempted"))
    if (not force) and last and now_ms - last < TRACE_LOG_SECONDS * 1000:
        return
    _G(_TRACE_LAST_KEY, now_ms)
    Log("[trace][cycle]",
        "phase=%s" % (ctx.get("console_phase") or "—"),
        "state=%s" % (ctx.get("state") or "—"),
        "spot=%s" % (ctx.get("spot") or "—"),
        "manual=%s errors=%s" % (ctx.get("manual_gate_status") or "—",
                                  ",".join(ctx.get("manual_context_errors") or []) or "—"),
        "vrp_context=%s" % (ctx.get("vrp_context_status") or "—"),
        "reason=%s" % (ctx.get("plan_build_reason") or "—"),
        "not_lockable=%s" % (ctx.get("not_lockable_reason") or "—"),
        "candidates=%s lockable=%s vrp_blocked=%s" % (
            ctx.get("display_candidates_count") or 0, ctx.get("lockable_candidates_count") or 0,
            ctx.get("plan_vrp_blocked") or 0),
        "diag=%s" % _diag_compact(ctx.get("enum_diag")))


# ---------- 计划预览（完整主链内置） ----------

def integrated_plan_preview(spot, market_context=None, portfolio_state=None):
    """整合执行流的 PLAN 段（执行会话式）：真实 _build_menu → GEX VRP_CONTEXT 有效性检查
    → 组合硬预算(给 portfolio_state 时) → 返回可锁定方案 + 各域裁决。

    v3.0.5 起 VRP_CONTEXT 只校验 GEX iv/rv rank 数据有效性，不按 rank 高低过滤候选。"""
    now_ms = _now_ms()
    ...
    if reason != "OK" or not menu:
        out["lockable"] = []
        return out
    if (market_context or {}).get("source") == "GEX_MONITOR_IV_RV_RANK":
...
def _manual_context_for_cycle(now_ms):
    if not MANUAL_PLANNING_ALLOWED:
        return None
    gex = fetch_gex_vrp_context(DIRECTION_BIAS)
    market_context = gex.get("market_context")
    vrp_context_status = gex.get("status")
    sig = manual_config_signature(
        MANUAL_PLANNING_ALLOWED, DIRECTION_BIAS, SHORT_DTE_HOURS, SHORT_DELTA_RANGE,
        PROTECTION_WIDTH_RANGE, ORDER_AMOUNT, MANUAL_AUDIT_CARD_ID,
        MANUAL_AUDIT_NOTE, MANUAL_CONTEXT_TTL_MIN, _manual_risk_policy(),
        market_context, vrp_context_status)
    ...
    if not ctx or ctx.get("config_signature") != sig:
        ctx = build_manual_context(
            ...
            _manual_risk_policy(), market_context, vrp_context_status)
...
def _apply_manual_context_to_ctx(ctx, manual_context, manual_check):
    ...
    if manual_context:
        ...
        ctx["vrp_context_status"] = manual_context.get("vrp_context_status")
        ctx["vrp_context"] = manual_context.get("market_context") or {}
    return ctx
...
def _dispatch_command(raw, meta, now_ms):
    ...
    if status == "UNKNOWN":
        ...
        _record_last_command(raw, cmd, "UNKNOWN", outcome, now_ms)
        return {"action": "UNKNOWN", "status": status, "outcome": outcome, "raw": raw}
    if status == "DUPLICATE":
        outcome = "duplicate_ignored"
        cmd_ledger_record(cmd, res["key"], "DUPLICATE", "ignored", now_ms)
        _record_last_command(raw, cmd, "DUPLICATE", outcome, now_ms)
        return {"action": cmd["type"], "status": status, "outcome": outcome, "raw": raw}
    outcome = _handle_command(cmd["type"], cmd, now_ms)
    cmd_ledger_record(cmd, res["key"], "ACCEPTED", outcome, now_ms)
    _record_last_command(raw, cmd, "ACCEPTED", outcome, now_ms)
    return {"action": cmd["type"], "status": status, "outcome": outcome, "raw": raw}
...
def _vrp_recheck_locked(locked, spot, amount, short_quote, protection_quote, manual_context):
    ...
    if not mc:
        return None, None
    if mc.get("source") == "GEX_MONITOR_IV_RV_RANK":
        ok = (manual_context or {}).get("vrp_context_status") == "VRP_CONTEXT_VALID"
        return bool(ok), {
            "schema_name": "GexVrpContextValidity",
            "source": mc.get("source"),
            "pass": bool(ok),
            "iv_rv_ratio": mc.get("iv_rv_ratio"),
            "iv_rv_rank_pct": mc.get("iv_rv_rank_pct"),
            "reason_codes": [] if ok else [(manual_context or {}).get("vrp_context_status")],
        }
    return None, {"error": "VRP_CONTEXT_UNSUPPORTED_SOURCE"}
...
def _attempt_commit(locked, spot, manual_context, now_ms):
    ...
    **跨轮持久重挂**（替代一次性追价）。预提交不过/门控关 → 仅空跑预览；两腿成交达标 → 冻结入场快照；
    ...
    if (prog.get("short_done") or 0.0) > (prog.get("prot_done") or 0.0) + 1e-12:
        ...
        return result
    if not pre["passed"] and (prog.get("short_done") or 0.0) > 1e-12:
        snap = _build_partial_vertical_snapshot(
            locked, prog, spot, now_ms,
            abandon_reason="PRECOMMIT_FAILED_AFTER_PARTIAL_SHORT")
        _G(_POSITION_KEY, snap)
        _G(_LOCKED_KEY, None)
        ledger_set_state(S_SHORT_ACTIVE_PROTECTED)
        result.update({"entry_snapshot": snap, "partial_position": True,
                       "reason": "ENTRY_PARTIAL_VERTICAL_MANAGED_PRECOMMIT_FAILED"})
        return result
    if not pre["passed"]:
...
def _stable_menu_meta_valid(meta, manual_context):
    """固定备选库是否仍属于当前人工上下文。只要方向/数量/默认范围/风险门配置变化，旧库失效。"""
...
def _store_stable_menu(menu, manual_context, now_ms, reason, diag, lockable_count,
    ...
    meta = {
        ...
        "vrp_blocked": vrp_blocked or 0,
        "not_lockable_reason": not_lockable_reason,
        "frozen": bool(PLAN_LIBRARY_FREEZE),
    }
    _G(_MENU_KEY, menu)
...
def run_cycle(now_ms=None):
    """Single manual-gate cycle: command, existing risk management, stable manual plan display, approval lock."""
    ...
    plan_reason = stable_meta.get("reason")
    ...
    if recovery.get("state") == "RECOVERY_BLOCKED":
        ...
        # 固定备选库：已有同一人工上下文的候选库时，不再因实时盘口/排序刷新而抖动。
        should_build = not bool(display_candidates)
        if (not PLAN_LIBRARY_FREEZE) and (not PLAN_LIBRARY_REFRESH_COMMAND_ONLY):
            last_build = int(_G(_LIB_BUILD_TS_KEY) or 0)
            should_build = bool(spot and (not lib or now_ms - last_build >= PLAN_REFRESH_SECONDS * 1000))

        if spot and should_build:
            ...
            if reason == "OK" and menu and market_context:
                ...
                not_lockable_reason = (manual_context or {}).get("vrp_context_status") or "VRP_CONTEXT_MISSING"
            ...
            stable_meta = _store_stable_menu(display_candidates, manual_context, now_ms, reason, diag,
                                             len(pending_tmp or []), plan_vrp_blocked, not_lockable_reason)
            _store_plan_trace(now_ms, reason=reason, diag=diag, menu_count=len(menu or []),
                              lockable_count=len(pending_tmp or []), vrp_blocked=plan_vrp_blocked,
                              not_lockable_reason=not_lockable_reason)
            menu_source = "built_frozen" if PLAN_LIBRARY_FREEZE else "rebuilt"
        elif display_candidates:
            menu_source = "frozen"
            _store_plan_trace(now_ms, reason=plan_reason, diag=enum_diag,
    ...
    if display_candidates and not pending and not_lockable_reason:
        display_candidates = _annotate_menu_lock_state(display_candidates, pending, not_lockable_reason)
        _G(_MENU_KEY, list(display_candidates or []))
    if plan_build_attempted:
        _trace_plan_candidates(display_candidates, spot, pending, not_lockable_reason)
    locked_display = None
    ...
    ctx["plan_library_frozen"] = bool(display_candidates and PLAN_LIBRARY_FREEZE)
    ...
    ctx["plan_build_reason"] = plan_reason or plan_trace.get("reason")
    ctx["enum_diag"] = enum_diag or plan_trace.get("diag")
    ctx["plan_build_attempted"] = plan_build_attempted
    ctx["plan_build_age_sec"] = ((now_ms - plan_trace.get("ts", now_ms)) / 1000.0
                                  if plan_trace.get("ts") else None)
    ctx["kill_new_risk"] = kill
    ctx["last_command"] = disp.get("action")
    ctx["last_command_outcome"] = disp.get("outcome")
    last_cmd = _G(_LAST_COMMAND_KEY) or {}
    if last_cmd:
        ctx["last_command_raw"] = last_cmd.get("raw")
        ctx["last_command_status"] = last_cmd.get("status")
        ctx["last_command"] = ctx["last_command"] or last_cmd.get("type")
        ctx["last_command_outcome"] = ctx["last_command_outcome"] or last_cmd.get("outcome")
    if commit_result:
...
def main():
    ...
    Log("[boot] S:PM manual-gate execution v%s" % STRATEGY_VERSION,
        ...
        "trace=%ss stages=%s" % (TRACE_LOG_SECONDS, TRACE_PLAN_STAGES))
