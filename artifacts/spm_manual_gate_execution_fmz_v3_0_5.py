...
# Deribit S:PM 垂直信用价差卖方执行链 v3.0.5-manual-gate（FMZ 单文件；单一 run_cycle 主链 + 交互控制台 + 对冲生命周期）
...
v3.0.5 使用 GEX Monitor BTC Metric API 的 iv/rv ratio rank 作为轻量 VRP_CONTEXT。
执行层仍只需打开 MANUAL_PLANNING_ALLOWED；出方案不交易由交易门关闭实现。
"""

# ===== 当前版本号（显示于启动日志/面板/合成文件头）=====
# 3.0.5-manual-gate：GEX iv/rv rank 只做 VRP_CONTEXT 数据有效性检查，不做价格硬门控。
