...
# Deribit S:PM 垂直信用价差卖方执行链 v3.0.6-manual-gate（FMZ 单文件；单一 run_cycle 主链 + 交互控制台 + 对冲生命周期）
...
v3.0.6 使用 GEX Monitor BTC Metric API 的 iv/rv ratio rank 作为轻量 VRP_CONTEXT，
并显式回显 FMZ 策略交互命令审计。
执行层仍只需打开 MANUAL_PLANNING_ALLOWED；出方案不交易由交易门关闭实现。
"""

# ===== 当前版本号（显示于启动日志/面板/合成文件头）=====
# 3.0.6-manual-gate：补齐 FMZ GetCommand 交互审计；确认码控件必须用 string。
STRATEGY_VERSION = "3.0.6-manual-gate"
...
def run_cycle(now_ms=None):
    ...
    if display_candidates:
        try:
            ctx.update(_flat_plan_fields(display_candidates[0]))
            ctx["preview_plan_detail"] = "stable_first_candidate"
        except Exception:
