...
# Deribit S:PM 垂直信用价差卖方执行链 v3.0.4-manual-gate（FMZ 单文件；单一 run_cycle 主链 + 交互控制台 + 对冲生命周期）
...
v3.0.4 固定备选方案库：首次扫描后稳定展示完整候选库，不随实时盘口反复重排；明确展示 VRP_CONTEXT_MISSING。
执行层仍只需打开 MANUAL_PLANNING_ALLOWED；出方案不交易由交易门关闭实现。
"""

# ===== 当前版本号（显示于启动日志/面板/合成文件头）=====
# 3.0.4-manual-gate：固定备选方案库显示，避免候选明细随刷新抖动。
