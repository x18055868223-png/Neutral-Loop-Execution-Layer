...
# Deribit S:PM 垂直信用价差卖方执行链 v3.0.3-manual-gate（FMZ 单文件；单一 run_cycle 主链 + 交互控制台 + 对冲生命周期）
...
v3.0.3 增加候选方案状态栏展示：即使 VRP context 缺失、不生成确认码，也会展示扫描到的方案明细。
执行层仍只需打开 MANUAL_PLANNING_ALLOWED；出方案不交易由交易门关闭实现。
"""

# ===== 当前版本号（显示于启动日志/面板/合成文件头）=====
# 3.0.3-manual-gate：在 v3.0.2 链路诊断基础上补齐候选/确认码/执行状态栏展示。
...
STATUS_MENU_SIZE = 10                   # 状态栏最多展示候选方案条数
...
def disp_menu_table(menu, selected_no, spot):
    ...
    return {
        "type": "table",
        "title": "候选方案明细（确认码为空=仅展示/未锁定；有效$=净 credit；信用/保证金=周期保证金回报）",
...
def disp_pipeline_table(ctx):
    ...
    rows = [
        ...
        ["候选展示", "展示%s / 可锁定%s / VRP阻断%s ｜ %s" % (
            g("display_candidates_count") or 0, g("lockable_candidates_count") or 0,
            g("plan_vrp_blocked") or 0, g("not_lockable_reason") or "—")],
        ["确认码", codes],
        ["预提交", pre_line],
...
def disp_status_panel(ctx, note=""):
    ...
    if ctx.get("menu"):                           # 候选方案即使不可锁定也展示，便于定位 VRP/预算/确认码链路
        tables.append(disp_menu_table((ctx["menu"] or [])[:STATUS_MENU_SIZE], None, ctx.get("spot")))
...
def run_cycle(now_ms=None):
    ...
    pending = []
    display_candidates = list(_G(_MENU_KEY) or [])
    ...
    if recovery.get("state") == "RECOVERY_BLOCKED":
        ...
        if spot and (not lib or now_ms - last_build >= PLAN_REFRESH_SECONDS * 1000):
            ...
            _store_plan_trace(now_ms, reason=reason, diag=diag, menu_count=len(menu or []),
                              lockable_count=len(lockable or []), vrp_blocked=plan_vrp_blocked,
                              not_lockable_reason=not_lockable_reason)
        if lib and lib.get("recommendations"):
            ...
            _codes = dict((c["id"], c["confirm_code"]) for c in pending)
            for _p in display_candidates:
                if _p.get("id") in _codes:
                    _p["_confirm_code"] = _codes.get(_p.get("id"))
            phase = "HARD_APPROVAL_WAIT"

    if display_candidates and not pending and not_lockable_reason:
        for _p in display_candidates:
            _p["_not_lockable_reason"] = not_lockable_reason
    ...
    if display_candidates:
        try:
            ...
            ctx["preview_plan_detail"] = "top_candidate"
        except Exception:
