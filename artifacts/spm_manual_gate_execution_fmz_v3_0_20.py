...
# Deribit S:PM 垂直信用价差卖方执行链 v3.0.20-manual-gate（FMZ 单文件；单一 run_cycle 主链 + 交互控制台 + 对冲生命周期）
...
STRATEGY_VERSION = "3.0.20-manual-gate"
...
ENTRY_MAX_ATTEMPTS = 20            # 开仓活动软等待上限；无成交不清锁，达到后保护腿可触发受控 taker
ENTRY_PROTECTION_ORDER_WAIT_SECONDS = 8    # 保护腿 maker 挂单存续；每轮按买一/标记价重评估
ENTRY_SHORT_ORDER_WAIT_SECONDS = 60        # 卖方腿 maker 挂单存续；实盘观察后延长等待
ENTRY_PROTECTION_TAKER_AFTER_ATTEMPTS = 20 # 保护腿超过软等待上限后才评估 taker 兜底
...
def validate_config():
    ...
    if ENTRY_PROTECTION_ORDER_WAIT_SECONDS < 1 or ENTRY_SHORT_ORDER_WAIT_SECONDS < 1:
        errs.append("ENTRY_*_ORDER_WAIT_SECONDS must be >=1")
    if ENTRY_PROTECTION_TAKER_AFTER_ATTEMPTS < 1:
        errs.append("ENTRY_PROTECTION_TAKER_AFTER_ATTEMPTS>=1")
    return errs
...
def entry_campaign_decision(has_locked, quotes_ok, credit_ok, attempts, max_attempts,
    ...
    """开仓活动下一状态 / 是否可下单（纯）。
    max_attempts 是保护腿 taker 兜底的软等待上限，不再代表清锁/放弃锁定方案。"""
    if not has_locked:
...
def exec_protection_maker_price(mark, best_bid, best_ask, tick, meta=None):
    """买保护腿：优先挂 mark；mark 低于买一则 join bid，且始终低于卖一避免 taker。"""
    cap = best_ask - tick if tick else best_bid
    base = max(mark, best_bid)
    return exec_round_order_price("buy", min(base, cap), meta, tick)
...
def _post_entry_protection_once(instrument, amount, maker_price, quote, attempt,
    ...
    q = quote or {}
    mark, ask, tick = q.get("mark"), q.get("best_ask"), q.get("tick") or 0.0
    can_try_taker = (attempt >= ENTRY_PROTECTION_TAKER_AFTER_ATTEMPTS
                     and mark is not None and ask is not None and tick > 0)
    if can_try_taker:
        ask_depth = _best_ask_depth(instrument, q)
        ask_within_one_tick = ask <= mark + tick + 1e-12
        depth_ok = ask_depth is not None and ask_depth + 1e-12 >= amount
        credit_ok = entry_net_credit(short_price, ask, amount, fees) >= credit_floor
        if ask_within_one_tick and depth_ok and credit_ok:
            return _post_taker_once("buy", instrument, amount, ask, label + "_taker",
                                    meta, quote=q, leg="protection")
        Log("[entry][prot] taker_guard hold ask=%s mark=%s tick=%s depth=%s amt=%s credit_ok=%s" %
            (ask, mark, tick, ask_depth, amount, credit_ok))
    return _post_maker_once("buy", instrument, amount, maker_price, label,
                            meta, quote=q, leg="protection",
                            wait_seconds=ENTRY_PROTECTION_ORDER_WAIT_SECONDS)
...
def _attempt_commit(locked, spot, manual_context, now_ms):
    ...
    **跨轮持久重挂**（替代一次性追价）。预提交不过/门控关 → 仅空跑预览；两腿成交达标 → 冻结入场快照；
    ENTRY_MAX_ATTEMPTS 作为保护腿 taker 兜底的软等待上限，不再把锁定方案打回方案库。"""
    lib = _G(_LIB_KEY)
